function [ PD,PF,Precision,F1,AUC,Accuracy,G_measure,MCC ] = TOMOFWTNB(Source0,Target,isDiscrete,sigma)
%TNB Summary of this function goes here
%   Detailed explanation goes here:根据target数据的属性信息，为training data set 中的每个训练样本赋予权重. 详见《Transfer learning for cross-company software defect prediction》
%
% INPUTS:
%   (1) Source - [S1;S2;...;SN]; each row of Si corresponds to a observation and the last column is label {0,1}.
...
% TOMO
if ~iscell(Source0)
    Source = TOMO(Source,Target(:,1:end-1),1);
end
...
try
    [ PD,PF,Precision,F1,AUC,Accuracy,G_measure,MCC ] = Performance(Target(:,end), pro_pos); % Call self-defined Performance()
catch
    PD=nan;PF=nan;Precision=nan;F1=nan;AUC=nan;Accuracy=nan;G_measure=nan;MCC=nan;
end
